...
def sample_and_group(xyz, points, M, radius, K, use_xyz=True):
    ...
    :return: new_xyz, shape=(B, M, 3); new_points, shape=(B, M, K, C+3);
             group_indexes, shape=(B, M, K); grouped_xyz, shape=(B, M, K, 3)
    '''
    new_xyz = gather_points(xyz, fps(xyz, M))
    grouped_indexes = ball_query(xyz, new_xyz, radius, K)
    grouped_xyz = gather_points(xyz, grouped_indexes)
    grouped_xyz -= torch.unsqueeze(new_xyz, 2).repeat(1, 1, K, 1)
    if points is not None:
        grouped_points = gather_points(points, grouped_indexes)
        if use_xyz:
    ...
    return new_xyz, new_points, grouped_indexes, grouped_xyz
...
def sample_and_group_all(xyz, points, use_xyz=True):
    ...
    :return: new_xyz, shape=(B, 1, 3); new_points, shape=(B, 1, M, C+3);
             group_indexes, shape=(B, 1, M); grouped_xyz, shape=(B, 1, M, 3)
    '''
    B, M, C = xyz.shape
    new_xyz = torch.zeros(B, 1, C)
    grouped_indexes = torch.arange(0, M).long().view(1, 1, M).repeat(B, 1, 1)
...
class PointNet_SA_Module(nn.Module):
    ...
    def forward(self, xyz, points):
        if self.group_all:
            new_xyz, new_points, grouped_indexes, grouped_xyz = sample_and_group_all(xyz, points, self.use_xyz)
        else:
            new_xyz, new_points, grouped_indexes, grouped_xyz = sample_and_group(xyz=xyz,
                                                                              points=points,
                                                                              M=self.M,
                                                                              radius=self.radius,
                                                                              K=self.K,
...
if __name__ == '__main__':
    ...
    M, radius, K = 5, 5, 6
    new_xyz, new_points, grouped_indexes, grouped_xyz = sample_and_group(xyz, points, M, radius, K)
    print(new_xyz[0])
    print(new_points[0])
    '''
    print('='*20, 'backbone', '='*20)
    M, radius, K, in_channels, mlp = 2, 0.2, 3, 6, [32, 64, 128]
    new_xyz, new_points, grouped_indexes = pointnet_sa_module(xyz, points, M, radius, K, in_channels, mlp)
    print('new_xyz: ', new_xyz.shape)
    print('new_points: ', new_points.shape)
    print('grouped_indexes: ', grouped_indexes.shape)
